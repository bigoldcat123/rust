...
impl Solution {
    ...
    pub fn max_average_ratio(classes: Vec<Vec<i32>>, mut extra_students: i32) -> f64 {
        #[derive(PartialEq,Eq,Debug)]
                ...
                let mut classes:BinaryHeap<Reverse<Dataa>> = classes.into_iter().filter(|x| x[0] != x[1]).map(|x| Reverse(Dataa{
                    pass:x[0],
                    total:x[1],
                })).collect();
                full-=classes.len();
                ...
                let s:f64 = classes.iter().map(|Reverse(x)| x.pass as f64 / x.total as f64).sum();
    ...
    fn abc() {
        ...
        impl PartialOrd for Task{
            fn partial_cmp(&self, other: &Self) -> Option<Ordering> {
                Some(other.priority.cmp(&self.priority).then(other.user_id.cmp(&self.user_id)))
            }
        }
        impl Ord for Task {
            fn cmp(&self, other: &Self) -> Ordering {
    ...
    pub fn min_number_of_seconds(mut mountain_height: i32, worker_times: Vec<i32>) -> i64 {
        ...
        impl PartialOrd for Worker {
            fn partial_cmp(&self, other: &Self) -> Option<Ordering> {
                self.current_cost.partial_cmp(&other.current_cost).map(|x| x.reverse())
            }
        }
        impl Ord for Worker {
            fn cmp(&self, other: &Self) -> Ordering {
        ...
        let mut heap = BinaryHeap::from_iter(worker_times.into_iter().map(|x| Worker::new(x as i64)));
        let mut res = 0;
    ...
    fn a() {
        ...
        fn merge_k_lists(lists: Vec<Option<Box<ListNode>>>) -> Option<Box<ListNode>> {
            let mut heap:BinaryHeap<L> = BinaryHeap::from_iter(lists.into_iter().map(|x| L { l: x }));
            ...
            impl FoodRatings {
                ...
                fn new(mut foods: Vec<String>,mut cuisines: Vec<String>,mut ratings: Vec<i32>) -> Self {
                    ...
                    for i in 0..len {
                        ...
                        c.entry(cuisine.clone()).or_insert(BTreeSet::new()).insert((rating.clone(),food.clone()));
                ...
                fn change_rating(&mut self, food: String, new_rating: i32) {
                    ...
                    if let Some(foods) = self.cuisines.get_mut(cuisine) {
                        ...
                        self.food_cuisine.insert(food, (cuisine.clone(),new_rating));
                    }
                }
            ...
            impl MedianFinder {
                ...
                fn find_median(&self) -> f64 {
                    if (self.big_min_heap.len() + self.small_max_heap.len()) % 2 == 0 {
                        (self.small_max_heap.peek().copied().unwrap() + self.big_min_heap.peek().copied().unwrap().0) as f64 / 2.0
                    }else {
        ...
        pub fn median_sliding_window(nums: Vec<i32>, k: i32) -> Vec<f64> {
            ...
            res[0] = if k % 2 == 0 {
                ((small_max_set.last().copied().unwrap().0 as f64 + (big_min_set.first().copied().unwrap().0) as f64)) / 2.0
            }else {
            ...
            impl MKAverage {
                ...
                fn add_element(&mut self, num: i32) {
                    if self.nums.len() >= self.m {
                        let del = (self.nums[self.nums.len() - self.m],self.nums.len() - self.m);
