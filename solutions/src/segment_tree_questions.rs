use crate::{segment_tree::{Mark, MyLazySegTree, MyLazySegTreeMultiMark, MyLazySumSegTree, MySegTree}, tree_array::TreeArray3};
...
impl BookMyShow {
    ...
    fn gather(&mut self, k: i32, max_row: i32) -> Vec<i32> {
        ...
        if l <= max_row {
            ...
            self.seat.update(l as usize, self.seat.nodes[l as usize - 1] - k);
...
pub fn falling_squares(positions: Vec<Vec<i32>>) -> Vec<i32> {
    ...
    positions.into_iter().map(|x| {
        let max = lazy_seg_tree.query_value(x[0] as usize + 1, x[0] as usize + x[1] as usize);
        lazy_seg_tree.update_range_value(x[0] as usize + 1, x[0] as usize + x[1] as usize, max + x[1]);
...
impl Fancy {
    ...
    fn append(&mut self, val: i32) {
        self.len += 1;
        self.tree.update_range_value(self.len, self.len, Mark::Add(val as usize));
    }

    fn add_all(&mut self, inc: i32) {
        if self.len > 0 {
            self.tree.update_range_value(1, self.len, Mark::Add(inc as usize));
        }
    }

    fn mult_all(&mut self, m: i32) {
        if self.len > 0 {
            self.tree.update_range_value(1, self.len, Mark::Mul(m as usize));
        }
